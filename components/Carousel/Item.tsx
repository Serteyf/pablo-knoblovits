...
const Item: React.FC<ItemPropsType> = ({ index, gap, children }) => {
  ...
  const handleKeyUp: React.KeyboardEventHandler<HTMLDivElement> | undefined = (event) => {
    if (!context) return;
  ...
  const handleKeyDown: React.KeyboardEventHandler<HTMLDivElement> | undefined = (event) =>
    event.key === "Tab" && setDidUserTab(true);
